...
class App extends Component {
  state = {
    score: 0,
    timer: 60,
    activeImageObject: imagesList[0],
  ...
  setTime = () => {
    ...
    if (timer === 0) {
      ...
      this.clear = this.setState(prev => ({timer: prev.timer - 1}))
    }
  }
  ...
  activeImage = () => {
    const {activeImageObject} = this.state
    return (
      <div className="active-image-container">
        <img
          className="active-image"
          src={activeImageObject.imageUrl}
          alt="match"
        />
      </div>
    )
  ...
  guessImage = id => {
    const {activeImageObject} = this.state
    if (activeImageObject.id === id) {
      this.setState(prev => ({score: prev.score + 1}))
      const imageIndex = Math.floor(Math.random() * imagesList.length)
      this.setState({activeImageObject: imagesList[imageIndex]})
    } else {
      this.setState({isRunning: false})
      clearInterval(this.clear)
    }
  ...
  thumbnailImages = () => {
    ...
    return (
      <ul className="thumbnail-list-container">
        {updatedImagesList.map(eachImage => (
          <Thumbnail
            eachImage={eachImage}
            key={eachImage.id}
  ...
  scoreboard = () => {
    ...
    return (
      <div className="scoreboard-container">
        ...
        />
        <p className="scoreboard-score-text">YOUR SCORE</p>
        <h1 className="scoreboard-value">{score}</h1>
        <button
          className="scoreboard-play-again-container"
          type="button"
        ...
        >
          <img
            ...
            alt="reset"
            className="scoreboard-reset-image"
          />
          PLAY AGAIN
        </button>
      </div>
  ...
  render() {
    const {score, timer, isRunning} = this.state
    return (
